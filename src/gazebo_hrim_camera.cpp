...
#include <hrim_sensor_camera_msgs/msg/image.hpp>
...
public:
  ...
  /// Image publisher.
  rclcpp::Publisher<hrim_sensor_camera_msgs::msg::Image>::SharedPtr image_pub_{nullptr};

  /// Camera info publisher.
  rclcpp::Publisher<hrim_sensor_camera_msgs::msg::CameraInfo>::SharedPtr camera_info_pub_{nullptr};
...
void GazeboRosCamera::Load(gazebo::sensors::SensorPtr _sensor, sdf::ElementPtr _sdf)
{
  ...
  impl_->image_pub_ = impl_->ros_node_->create_publisher<hrim_sensor_camera_msgs::msg::Image>(
    impl_->camera_name_ + "/image_raw");

  // Camera info publisher
  // TODO(louise) Migrate ImageConnect logic once SubscriberStatusCallback is ported to ROS2
  impl_->camera_info_pub_ = impl_->ros_node_->create_publisher<hrim_sensor_camera_msgs::msg::CameraInfo>(
    impl_->camera_name_ + "/image_raw/camera_info");
  ...
  // Dynamic reconfigure
//    dyn_srv_ =
//      new dynamic_reconfigure::Server<gazebo_hrim_plugins::GazeboRosCameraConfig>
//      (*this->rosnode_);
//    dynamic_reconfigure::Server<gazebo_hrim_plugins::GazeboRosCameraConfig>
//      ::CallbackType f =
  ...
  const std::string & /*_format*/)
{
  ...
  image_msg.step = impl_->skip_ * _width;
  size_t st0 = (impl_->skip_ * _width * image_msg.height);
  image_msg.data.resize(st0);
  memcpy(&image_msg.data[0], reinterpret_cast<const void *>(_image), st0);
  ...
  if (focal_length == 0) {
    ...
    RCLCPP_WARN(impl_->ros_node_->get_logger(),
      ...
      focal_length, impl_->camera_name_.c_str(), this->width, hfov, computed_focal_length);
  }

  // CameraInfo
  hrim_sensor_camera_msgs::msg::CameraInfo camera_info_msg;
